	...
	if SERVER then
		...
		timer.Simple(4.0, function()
			...
			for _, ply in ipairs(player.GetAll()) do
				...
				ply:Spawn()
				ply:GodEnable()
				ply:SetPos(ply:GetPos() + ply:GetAimVector() * math.random(0,150)) --for some reason, this is needed to prevent the players from spawning in the same spot
...
net.Receive("spectate_next", function(len,ply)
	if not contains(alive_people,ply) and #alive_people > 1 then
		...
		while true do
			...
			if ply:GetObserverTarget() != randomPly and randomPly:GetObserverMode() == OBS_MODE_NONE then break end
			if #alive_people <= 0 then break end
		end
		ply:Spawn()
